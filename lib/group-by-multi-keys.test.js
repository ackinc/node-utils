...
describe("groupByMultiKeys", () => {
  ...
  test("does not overwrite any existing keys in record objects when adding the temporary group key to them", () => {
    let grouped;

    const recordsCopy = records.map((r) => ({ ...r }));
    grouped = groupByMultiKeys(recordsCopy, (user) => user.emails);
    grouped.forEach((group) =>
      group.forEach((recordAfterGrouping) => {
        const { id } = recordAfterGrouping;
        const correspondingRecordBeforeGrouping = records.find(
          (r) => r.id === id
        );

        expect(recordAfterGrouping).toStrictEqual(
          correspondingRecordBeforeGrouping
        );
      })
    );

    // because the temporary group key is added with attrname '__groupKey' by default
    const recordsWithAGroupKeyAttributeAdded = records.map((r) => ({
      ...r,
      __groupKey: "Some nonsense",
    }));
    const recordsWithAGroupKeyAttributeAddedCopy = recordsWithAGroupKeyAttributeAdded.map(
      (r) => ({ ...r })
    );
    grouped = groupByMultiKeys(
      recordsWithAGroupKeyAttributeAddedCopy,
      (user) => user.emails
    );
    grouped.forEach((group) =>
      group.forEach((recordAfterGrouping) => {
        const { id } = recordAfterGrouping;
        const correspondingRecordBeforeGrouping = recordsWithAGroupKeyAttributeAddedCopy.find(
          (r) => r.id === id
        );
